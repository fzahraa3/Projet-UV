...
#FENÊTRE PRINCIPALE
root = tk.Tk()
root.title("Analyse de l'exposition solaire")
...
titre = tk.Label( main_frame,text="Analyseur d'exposition solaire", font=("Arial", 24, "bold"),bg="#f4f4f4", fg="#222")
titre.pack(pady=20)
resultats_label = tk.Label(main_frame,text="",font=("Arial", 12),bg="#f4f4f4",justify="left")
resultats_label.pack(pady=10)
graph_frame = tk.Frame(main_frame, bg="#f4f4f4")
...
resultats_label.config( text=f"Voici vos résultats !\nLe {jour} {mois}, au pays sélectionné, à {heure}h,le Soleil se trouve à {round(angle_solaire_deg,2)} degrés.")
...
plt.title("CAS LE PLUS ENSOLEILLÉ - Égypte (Phototype I, UV de 11, à partir de midi)")
plt.xlabel("Moment de la journée (heures)")
plt.ylabel("Niveau de brûlure")
...
if temps_max_pire is None:
    print("Dans le scénario le plus ensoleillé, vous ne dépassez pas le seuil de brûlure avant le coucher du soleil.")
...
plt.title("CAS LE PLUS ENSOLEILLÉ - Égypte (Phototype VI, UV de 11, à partir de midi)")
plt.xlabel("Moment de la journée (heures)")
plt.ylabel("Niveau de brûlure")
plt.xlim(min(x_pire), max(x_pire))
plt.xticks(np.arange(min(x_pire), max(x_pire) + 0.5, 0.5))
